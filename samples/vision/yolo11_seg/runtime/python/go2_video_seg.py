...
"""
YOLO11-Seg 实例分割视频流实时推理脚本（宇树机器人 Go2 摄像头版）

本脚本通过宇树 SDK VideoClient 获取 Go2 机器人前置摄像头画面，
对每一帧进行 BPU 推理，实时绘制检测框、实例掩码和轮廓，并在窗口中显示结果。
...
工作流程:
    ...
    5) 预处理 -> BPU 推理 -> 后处理（检测框 + 掩码）。
    6) 绘制检测框、掩码、轮廓，并叠加 FPS 信息。
    7) 实时显示结果，并可选保存输出视频。
...
用法示例:
    ...
    # 指定网卡
    python video_seg.py --net-interface ens37

    # 保存输出视频
    python video_seg.py --save-video output_result.mp4
"""
